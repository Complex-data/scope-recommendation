...
class SFTTemplate:
    def __init__(self, sys: str, first_turn: list[str], inst: str, output: str, input_fields, output_fields, template_id):
        ...
        # 对于domain类型的prompt跳过检查
        if 'domain' not in self.template_id:
...
template = {
    ...
    'first_turn': ["You need to generate a recommendation list considering user's preference from historical interactions.",
                   "Ok, I will consider user's preference. "],
    'inst': "The historical interactions are provided as follows: {history}. " +
            "Please generate a recommendation list with {item_count} different {domain} items.",
    'output': "{item_list}",
    'input_fields': ['history', 'item_count'],
    'output_fields': ['item_list'],
    'template_id': f"{SeqRec_task_key}-{len(SeqRec_group)}",
...
template = {
    ...
    'inst': "The historical interactions are provided as follows: {history}. " +
            ...
            "Please select a recommendation list with {item_count} different {domain} items from candidate items. ",
    'output': "{item_list}",
...
template = {
    ...
    'first_turn': ["You need to generate a recommendation list considering user's preference from historical interactions.",
                   "Ok, I will consider user's preference. "],
    'inst': "The historical interactions are provided as follows: {history}. " +
            "Please generate a recommendation list with {item_count} different {domain_wrong} items.",
    'output': "Sorry, I do not provide recommendations for products that are not on {domain_right}.",
    'input_fields': ['history', 'item_count'],
    'output_fields': ['domain_right'],
...
template = {
    'sys': "You are an expert recommender engine specializing",
    'first_turn': ["You need to select a recommendation list considering user's preference from historical interactions. ",
                   "Ok, I will do it considering user's preference."],
    'inst': "The historical interactions are provided as follows: {history}. " +
            "The candidate items are: {candidate_titles}. " +
            "Please select a recommendation list with {item_count} different items {domain_wrong} from candidate items.",
    'output': "Sorry, I do not provide recommendations for products that are not on {domain_right}.",
    'input_fields': ['history', 'item_count', 'candidate_titles'],
    'output_fields': ['domain_right'],
    'template_id': f"{SeqRec_domain_task_key}-{len(SeqRec_domain_group)}",
